...
__all__ = ['get_optim_cfg', 'LitModel', 'BinLitModel', 'PLitModel', 'load_lit_state_dict', 'get_trainer']
...
class LitModel(LightningModule):
    def __init__(self, model, optim_cfg=None, num_classes=None, loss=FocalLoss()):
        super().__init__()
        self.model = model
        self.loss_fn = loss
...
def get_trainer(exp_name, gpus=1, max_epochs=40, distributed=False,
        monitor=dict(metric="val_acc", mode="max"), save_every_n_epochs=1, save_top_k=1, use_version=True, local_rank=0,
    **trainer_kawrgs):

    now = datetime.now() + timedelta(hours=7)
    root_log_dir = osp.join(
            "lightning_logs", exp_name)
    version = now.strftime("%b%d_%H_%M_%S")
    if use_version:
    ...
    trainer = Trainer(
        ...
        callbacks=[callback_ckpt, callback_tqdm, callback_lrmornitor] if local_rank == 0 else [],
